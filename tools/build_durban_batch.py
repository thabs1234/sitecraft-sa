#!/usr/bin/env python3
"""SiteCraft SA — Durban batch builder (cron, Day 7+).

Pulls REAL independent Durban businesses that have NO website and a phone from
...
outreach-tracker.csv (send_status=queued -> staged for MANUAL send, 4-5/day).
...
# Durban metro suburbs (independent-business density, low chain overlap)
SUBURBS = {
    "Berea": (-29.8579, 31.0218),
    "Morningside": (-29.8280, 31.0220),
    "Glenwood": (-29.8710, 31.0020),
    "Umbilo": (-29.8860, 30.9800),
    "Durban North": (-29.8160, 31.0560),
    "Westville": (-29.8430, 30.9450),
    "Pinetown": (-29.8230, 30.8700),
    "Kloof": (-29.7900, 30.8100),
    "Chatsworth": (-29.9100, 30.9700),
    "Umhlanga": (-29.7260, 31.0710),
}
RADIUS = 3000
...
def sample_html(name, cat, phone, suburb, lat, lon):
    ...
    cat_t = type_label(cat, {"shop": cat if cat != "shop" else None, "amenity": cat if cat != "shop" else None})
...
def main():
    ...
    for area, (lat, lon) in SUBURBS.items():
        ...
        for e in j.get("elements", []):
            ...
            if "*" in (tags.get("phone", "") or tags.get("contact:phone", "")):
                continue
            # must have NO website (truthful "you don't have a website" hook)
        ...
        time.sleep(2)
    # sort: prefer non-generic categories, then by area
    prospects.sort(key=lambda p: (p["cat"] == "shop" or p["cat"] == "default", p["area"]))
    # cap at 24 for a clean daily batch
    prospects = prospects[:24]
    ...
    for p in prospects:
        s = slug(p["name"])
        # ensure unique slug within batch
        base = s
        i = 2
        ...
        html_txt = sample_html(p["name"], p["cat"], p["phone"], p["area"], p["lat"], p["lon"])
    ...
    with open(CSV_PATH, "a", newline="", encoding="utf-8") as f:
        ...
        for r in rows:
            w.writerow(r)
    # record json
    with open(os.path.join(ROOT, "tools", "prospect_durban_20260816.json"), "w", encoding="utf-8") as f:
        json.dump(prospects, f, ensure_ascii=False, indent=2)

    print(f"Sample sites written: {written} -> samples/durban/")
